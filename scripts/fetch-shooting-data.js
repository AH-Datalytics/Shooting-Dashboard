...
try {
  ...
  console.warn('⚠️  pdfjs-dist not available - PDF scraping disabled');
}
...
// ─── BUFFALO GIVE DASHBOARD SCRAPER ───────────────────────────────────────
async function fetchBuffaloData(browser) {
  console.log('\n📊 Fetching Buffalo data from GIVE Dashboard...');
  ...
  try {
    ...
    // ─── NAVIGATE TO SHOOTING ACTIVITY TAB ───────────────────────────────
    ...
    try {
      ...
      console.log('  ✓ Clicked Shooting Activity tab');
      await page.waitForTimeout(2000);
    } catch (e) {
      console.log('  ⚠️  Could not click Shooting Activity tab by text, trying element search...');
      ...
      for (let tab of tabs) {
        ...
        if (text && text.toLowerCase().includes('shooting') && text.toLowerCase().includes('activity')) {
          await tab.click();
          console.log('  ✓ Found and clicked Shooting Activity tab');
          found = true;
          await page.waitForTimeout(2000);
          break;
        }
    ...
    // ─── SELECT BUFFALO CITY PD JURISDICTION ─────────────────────────────
    ...
    try {
      ...
      console.log('  ✓ Selected Buffalo City PD by text');
      buffaloSelected = true;
      await page.waitForTimeout(2000);
    } catch (e) {
      console.log('  ⚠️  Direct selection failed, trying dropdown approach...');
        ...
        for (let filterEl of filterElements) {
          ...
          if (text && (text.includes('Buffalo') || text.includes('Jurisdiction') || text.includes('Agency') || text.includes('Department'))) {
            ...
            if (options.length > 0) {
              ...
              console.log('  ✓ Selected Buffalo City PD from dropdown');
              buffaloSelected = true;
              await page.waitForTimeout(2000);
              break;
            }
    ...
    if (ytd2026 > 0 || ytd2025 > 0) {
      ...
      console.log('  ⚠️  Could not extract numbers from dashboard');
    ...
    console.error('  ✗ Error scraping Buffalo dashboard:', err.message);
    return null;
  } finally {
    await page.close();
  }
...
// ─── PDF SCRAPER (existing functionality) ───────────────────────────────────
async function fetchPdfData(pdfUrl, cityId) {
  if (!PDFParser) {
    console.log(`  ⊘ ${cityId}: pdfjs-dist not installed, skipping PDF scraping`);
    return null;
  }
  ...
  try {
    ...
    console.log(`  ⚠️  Could not parse data for ${cityId}`);
    return null;

  } catch (err) {
    console.error(`  ✗ ${cityId} error: ${err.message}`);
    return null;
  }
}
...
async function main() {
  ...
  try {
    ...
    if (buffaloResult && buffaloResult.success) {
      ...
      console.log('✗ Buffalo fetch failed, preserved existing data\n');
    }
